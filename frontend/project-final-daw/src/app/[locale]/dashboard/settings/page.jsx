...
import BlurToggle from "@/components/BlurToggle.jsx";
import { useTheme } from "@/app/context/ThemeContext.js";
import { useBlur } from "@/app/context/BlurContext.js";
import { Plus, PencilLine, Eye, EyeOff } from "lucide-react";
import FormModifyPercentage from "./components/FormModifyPercentage.jsx";
import ChangePasswordModal from "./components/ChangePasswordModal.jsx";
import ChangeNicknameModal from "./components/ChangeNicknameModal.jsx";
...
export default function SettingsPage() {
    ...
    return (
        <>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between py-3">
                                    <div>
                                        <p className="font-medium text-slate-900 dark:text-slate-100">{t("blurSensitiveData")}</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">{t("blurDescription")}</p>
                                    </div>
                                    <div className="group top-4 right-4 flex justify-center items-center border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-700 rounded-full p-2 cursor-pointer"
                                        onClick={blurToggle}
                                    >
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-700">
                                    ...
                                    <div>
                                        <button
                                            ...
                                            className="flex items-center gap-2 h-10 px-3 sm:px-4 rounded-xl hover:gap-4 transition-all duration-300 shadow-lg hover:shadow-xl group bg-slate-300 dark:bg-slate-600 text-slate-500 dark:text-slate-100 text-sm sm:text-base flex-shrink-0 border-none outline-none">
                                ...
                                <div className="flex items-center justify-between py-3">
                                    ...
                                    <div>
                                        <button
                                            onClick={() => setIsFormModifyPercentageOpen(true)}
                                            className="flex items-center gap-2 h-10 px-3 sm:px-4 rounded-xl hover:gap-4 transition-all duration-300 shadow-lg hover:shadow-xl group bg-slate-300 dark:bg-slate-600 text-slate-500 dark:text-slate-100 text-sm sm:text-base flex-shrink-0 border-none outline-none">
                                            <PencilLine className="w-4 h-4 sm:w-5 sm:h-5" />
            ...
            {/* Modal de modificar porcentajes y nómina */}
            {
            ...
            {/* Modal de cambiar nickname */}
            <ChangeNicknameModal
                isOpen={isChangeNicknameOpen}
                onClose={() => setIsChangeNicknameOpen(false)}
            />
        </>
    );
}